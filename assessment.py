...
def half_with_most_transactions_by_sessions(df):
    ...
    sorted_by_sessions_df = df.sort_values(by = ['Sessions'])
    count = sorted_by_sessions_df['Sessions'].count()
    shorter_sessions_half_df = sorted_by_sessions_df[ : count//2]
    longer_half_sessions_df = sorted_by_sessions_df[count//2 : ]
...
def avarage_session_duration(df, number):
    ...
    return aggr_duration/aggr_sessions